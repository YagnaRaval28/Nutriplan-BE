...
def _build_plan_response(plan: DietPlan, db: Session) -> dict:
    ...
    dietician_name = creator.name if creator else None
    ...
    for meal in plan.meals:
        ...
        for item in meal.items:
            ...
            ratio = float(item.quantity) / float(food.serving_size) if food and food.serving_size else 1
            items_out.append({
                ...
                "calories": round(float(food.calories) * ratio, 2) if food else (float(item.calories) if item.calories else None),
                "protein_g": round(float(food.protein_g) * ratio, 2) if food else 0,
                "carbs_g": round(float(food.carbs_g) * ratio, 2) if food else 0,
                "fats_g": round(float(food.fats_g) * ratio, 2) if food else 0,
                "notes": item.notes,
            })
        meals_out.append({
            "id": str(meal.id),
            "day_of_week": meal.day_of_week,
            "day_number": meal.day_of_week,
...
# ── Specific routes BEFORE wildcard /{plan_id} ────────────────────────────────
